...
class MyEsception : public exception
{
	...
	int dateState{-1};
public:
	MyEsception(const char* message, int dateState) : reason(message), dateState(dateState) {} // на случай если нужно передать код ошибки который будет подробно описан в документации на класс (библиотеку, программу...)
	explicit MyEsception(const char* message) :reason(message) {}
