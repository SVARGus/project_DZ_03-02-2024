...
void Apartment::freeRegistrPerson(Person& person) // переделал - надо проверить
{
	...
	if (person != 0)
	{
		...
		std::cout << " ƒл€ проверки - персона уже прописалась ранее" << std::endl;
}
...
void Home::freeRegistrPersonInHome(Person& person, const int floor, const int numbeOfApartmenOnFloor)
{
	...
	{
		std::cout << "¬ы выбрали не верный этаж, в доме всего этажей: " << this->floor << std::endl;
		return;
	}
	if (numbeOfApartmenOnFloor < 1 || numbeOfApartmenOnFloor > this->numbeOfApartmenOnFloor)
	{
		std::cout << "¬ы выбрали не верную квартиру на этаже, на этаже всего квартир: " << this->numbeOfApartmenOnFloor << std::endl;
		return;
	}
